...
class Migrator {
  ...
  migrateConfig(config, logger) {
    logger.info('Checking config for upgrade, starting state: $$$', config);
    config = config || { version: 0 };
    if (!this._versions.find(version => version.version >= config.version)) {
      throw new Error(`Unrecognised schema state ${config.version} - cannot upgrade.`);
    }

    return this._versions
      .filter(version => version.version > config.version)
...
const migrator = new Migrator()
  .skipToVersion(0.3)
  .overwriteProperty('genderPronouns', utils.deepClone(oneSixConfig).genderPronouns)
  .skipToVersion(1.1)
  .moveProperty('autoHD', 'sheetEnhancements.autoHD')
  .moveProperty('rollHPOnDrop', 'sheetEnhancements.rollHPOnDrop')
  .skipToVersion(1.3)
  .moveProperty('newCharSettings.savingThrowsHalfProf', 'newCharSettings.houserules.savingThrowsHalfProf')
  .moveProperty('newCharSettings.mediumArmorMaxDex', 'newCharSettings.houserules.mediumArmorMaxDex')
  .transformConfig(config => {
    _.defaults(config, oneSixConfig);
    _.defaults(config.tokenSettings, oneSixConfig.tokenSettings);
    _.defaults(config.newCharSettings, oneSixConfig.newCharSettings);
    _.defaults(config.advTrackerSettings, oneSixConfig.advTrackerSettings);
    _.defaults(config.sheetEnhancements, oneSixConfig.sheetEnhancements);
    return config;
