  ...
  buildHeader(entityName, criteria, suppliedCriteria, entitySpecificOptions, entityRecord) {
    const criteriaList = criteria.map((criterion) => {
      ...
      const valueList = criterion.values.map((value) => {
        ...
        const valueText = criterion.getValueText ? criterion.getValueText(value) : value;
        return `<a href="!shaped-list-${entityName} ${newOpts} ${entitySpecificOptions}" class="${className}">` +
          `${valueText}</a>`;
      }).join(', ');
      const name = Utils.toTitleCase(criterion.displayName);
      const nameHTML = !_.isEmpty(name) ? `<span class="criterion-name">${name}. </span>` : '';
      return `<div class="criterion">${nameHTML}${valueList}</div>`;
    }).join('');
