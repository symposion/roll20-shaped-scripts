...
class Importer extends ShapedModule {
  ...
  importSpellListFromJson(options) {
    ...
    const spellCount = this.roll20
      .findObjs({ type: 'attribute', characterid: options.character.id })
      .filter(attr => attr.get('name').match(/^repeating_spell\d.*_name$/))
      .length;

    if (spellCount + spells.length > 35) {
      this.reportError(`Import would give character ${options.character.get('name')} too many spells which will ` +
        'seriously degrade the performance of the sheet and your campaign more generally. In addition, it will place' +
        'excessive load on the API server which will eventually result in the Companion Script being sanctioned. ' +
        'Please limit spells to those which you actually use. The wider performance issue should be taken up with ' +
        'Roll20 directly.');
      return null;
    }
