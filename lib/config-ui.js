...
class ConfigUi {
  ...
  getConfigOptionGroupAdvTracker(config, optionsSpec) {
    ...
    const optionRows = this.makeToggleSetting(config, `${ats}.showMarkers`, 'Show Markers') +
  ...
  getConfigOptionGroupNewCharSettings(config, optionsSpec) {
    const optionRows = this.makeQuerySetting(config, 'newCharSettings.sheetOutput', 'Sheet Output',
      ...
      this.makeToggleSetting(config, 'newCharSettings.savingThrowsHalfProf', 'Half Proficiency Saves') +
      this.makeQuerySetting(config, 'newCharSettings.mediumArmorMaxDex', 'Medium Armor Max Dex',
        optionsSpec.newCharSettings.mediumArmorMaxDex());
  ...
  static validStatusMarkers() {
    const markers = ['red', 'blue', 'green', 'brown', 'purple', 'pink', 'yellow', 'dead', 'skull', 'sleepy',
