...
function ShapedScripts(logger, myState, roll20, parser, entityLookup, reporter, srdConverter, sanitise, mpp) {
    ...
    _.each(utils.flattenObject(_.omit(myState.config.newCharSettings, 'tokenActions')), (value, key) => {
      ...
      if (attrName) {
        ...
        if (value === '***default***' || (_.isBoolean(value) && !value)) {
          ...
          attribute.set('current', _.isBoolean(value) ? 1 : value);
        }
        if (attrName === 'shaped_d20') {
          if (value === '1d20') {
            roll20.setAttrByName(character.id, 'roll_setting', 'roll2');
          }
          else {
            const rollSettingAttr = roll20.getAttrObjectByName(character.id, 'roll_setting');
            if (rollSettingAttr) {
              rollSettingAttr.remove();
            }
          }
        }
      }
  ...
  this.handleD20Roll = function handleD20Roll(options) {
    if (options['2d20kl1'] || options['2d20kh1']) {
