...
function ShapedScripts(logger, myState, roll20, parser, entityLookup, reporter, srdConverter, sanitise, mpp) {
  ...
  this.configure = function configure(options) {
    ...
    let menu;
    if (options.atMenu) {
      menu = cui.getConfigOptionGroupAdvTracker(myState.config, ShapedConfig.configOptionsSpec);
    }
    else if (options.tsMenu) {
      menu = cui.getConfigOptionGroupTokens(myState.config, ShapedConfig.configOptionsSpec);
    }
    else if (options.ncMenu) {
      menu = cui.getConfigOptionGroupNewCharSettings(myState.config, ShapedConfig.configOptionsSpec);
    }
    else if (options.barMenu) {
      menu = cui.getConfigOptionGroupTokenBars(myState.config, ShapedConfig.configOptionsSpec);
    }
    else if (options.auraMenu) {
      menu = cui.getConfigOptionGroupTokenAuras(myState.config, ShapedConfig.configOptionsSpec);
    }
    else if (options.hrMenu) {
      menu = cui.getConfigOptionsGroupNewCharHouserules(myState.config, ShapedConfig.configOptionsSpec);
    }
    else if (options.seMenu) {
      menu = cui.getConfigOptionGroupSheetEnhancements(myState.config, ShapedConfig.configOptionsSpec);
    }
    else if (options.varsMenu) {
      menu = cui.getConfigOptionGroupVariants(myState.config, ShapedConfig.configOptionsSpec);
    }
    else {
  ...
  this.oldStyleModulesConfigure = function oldStyleModulesConfigure(cp) {
    ...
    return cp
      ...
      .option('atMenu', ShapedConfig.booleanValidator)
      .option('tsMenu', ShapedConfig.booleanValidator)
      .option('ncMenu', ShapedConfig.booleanValidator)
      .option('seMenu', ShapedConfig.booleanValidator)
      .option('hrMenu', ShapedConfig.booleanValidator)
      .option('barMenu', ShapedConfig.booleanValidator)
      .option('varsMenu', ShapedConfig.booleanValidator)
      .option('auraMenu', ShapedConfig.booleanValidator)
