...
function ShapedScripts(logger, myState, roll20, parser, entityLookup, reporter, srdConverter, sanitise, mpp) {
  ...
  this.configure = function configure(options) {
    // drop "menu" options
    utils.deepExtend(myState.config, _.omit(options, ['atMenu', 'tsMenu', 'ncMenu', 'seMenu', 'hrMenu']));
    ...
    if (options.atMenu || options.advTrackerSettings) {
      ...
      if (options.hrMenu) {
        menu = cui.getConfigOptionsGroupHouserules(myState.config, configOptionsSpec);
      }
      else {
  ...
  this.handleAdvantageTracker = function handleAdvantageTracker(options) {
    ...
    if (options.normal) { type = 'normal'; }
    else if (options.advantage) { type = 'advantage'; }
    else if (options.disadvantage) { type = 'disadvantage'; }

    if (!_.isUndefined(type)) { at.setRollOption(type, options.selected.character); }

    if (options.revert) { at.setAutoRevert(true, options.selected.character); }
    else if (options.persist) { at.setAutoRevert(false, options.selected.character); }
  };
  ...
  this.handleD20Roll = function handleD20Roll(options) {
    if (options.disadvantage || options.advantage) {
      const char = roll20.getObj('character', options.character.get('_id'));
      const autoRevertOptions = roll20.getAttrByName(options.character.get('_id'), 'auto_revert_advantage');
      if (autoRevertOptions === 'on') {
        at.setRollOption('normal', [char]);
      }
    }
  };
  ...
  this.checkInstall = function checkInstall() {
    ...
    if (myState.version !== schemaVersion) {
      ...
      switch (myState && myState.version) {
        ...
        case 1.2:
        case 1.3:
          // These lines are 1.3 upgrade specific - do not move down
          myState.config.newCharSettings.houserules = {};
          myState.config.newCharSettings.houserules.savingThrowsHalfProf =
            myState.config.newCharSettings.savingThrowsHalfProf;
          myState.config.newCharSettings.houserules.mediumArmorMaxDex =
            myState.config.newCharSettings.mediumArmorMaxDex;
          delete myState.config.newCharSettings.savingThrowsHalfProf;
          delete myState.config.newCharSettings.mediumArmorMaxDex;
        case 1.4:
