...
class AdvantageTracker {
  ...
  handleTokenChange(token) {
    ...
    if (this.shouldShowMarkers() && token.get('represents') !== '') {
      ...
      token.set(`status_${disadvantageMarker}`, setting === '@{roll_disadvantage}');
      token.set(`status_${advantageMarker}`, setting === '@{roll_advantage}');
    }
  }
  ...
  setRollOption(type, selectedChars) {
    ...
    _.each(resources, (resource) => {
      ...
      this.setStatusMarkers(resources.token, type === 'advantage', type === 'disadvantage');
