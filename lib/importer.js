...
class Importer extends ShapedModule {
    ...
    const importedPromisesList = _.chain(monsters)
      .map((monsterData) => {
        ...
        return characterProcessors.reduce((charPromise, proc) =>
          ...
          , Promise.resolve(character)).catch(this.errorHandler);
      })
      .compact()
      .value();
  ...
  importData(character, data) {
    ...
    if (!this.roll20.getAttrByName(character.id, 'version')) {
      charPromise = this.runImportStage(character, { sheet_opened: 1 }, 'Creating character');
    }
